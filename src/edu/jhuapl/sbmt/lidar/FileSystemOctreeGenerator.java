...
public class FileSystemOctreeGenerator
{
    ...
    public FileSystemOctreeGenerator(Path nearsdcRootDirectory, Path relativeTreeConstructionDirectory, int maxNumberOfPointsPerLeaf, BoundingBox bbox, int maxNumFiles) throws IOException
    {
    ...
    public void addPointsFromFileToRoot(Path inputFilePath, int nmax) throws IOException {  // first add all points to the root node, then expand the tree
    ...
    public void expandNode(FileSystemOctreeNode node) throws IOException {  // depth-first recursion, so we limit the number of open output files to 8
    ...
    public static void main(String[] args) throws IOException
    {
        if (args.length!=5) {
            ...
            System.out.println("   Path where tree is to be built, relative to nearsdc data path, e.g. GASKELL/RQ36_V3/OLA/tree");
        ...
        System.out.println();
        FileSystemOctreeGenerator tree=new FileSystemOctreeGenerator(nearsdcDataRootDirectory,treeRootDirectory, maxPointsPerLeaf, bbox, maxNumFiles);
        //
        ...
        for (int i=0; i<numFiles; i++) {
            ...
            System.out.println("Total points written so far = "+tree.totalPointsWritten);// TODO: close down all DataOutputStreams
            System.out.println("Total MB written so far = "+tree.convertBytesToMB(tree.root.getDataFilePath().toFile().length()));
            System.out.println();
            sw.reset();
        }
